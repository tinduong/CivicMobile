...
public class TextToSpeechService : ITextToSpeech
{
    ...
    public async Task Speak(string text, string language, CancellationTokenSource cancellationTokenSource)
    {
        ...
        var speakLanguage = locales.FirstOrDefault(item => item.Language.Contains(language));
        await TextToSpeech.Default.SpeakAsync(text, new SpeechOptions { Locale = speakLanguage }, cancellationTokenSource.Token);
    }
}